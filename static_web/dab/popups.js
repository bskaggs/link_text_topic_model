...
function listLinks(wikitext, navpop, titleToEdit, waiting) {

  var oldTarget =  navpop.originalArticle;
  var text = $(navpop.parentAnchor).text();
  ...
  for (var i=1; i<splitted.length; i=i+3) {
    if (typeof splitted[i] == typeof 'string' && splitted[i].length>0 && !omitRegex.test(splitted[i])) {
      possibilities.push(splitted[i]);
    } /* if */
  } /* for loop */
  console.log(navpop);
  var links = [];
  var i = 0;
  links.push({"title":friendlyCurrentArticleName, "text":text})
  $("#bodyContent a").each(function (index,y) {
    if (y.title && y.title != "" && y.getAttribute('class') != "new") {
      var yText = $(y).text();
      links.push({"title":y.title, "text":yText});
    }
  });
  console.log(links);
  console.log(i);
  var pageTitle = (titleToEdit || wgPageName.split('_').join(' '));
  $.getJSON(rootUrl + "dab?callback=?", { from: pageTitle, links: links, i: i }, function(json){
    var ret = [];
    
    for (var i=0; i<json.length; i++) {
      ret.push( "<li>" + retargetDab(json[i][0], oldTarget, friendlyCurrentArticleName, titleToEdit) + " <small>" + json[i][1] + "</small></li>");
    } /* for loop */
    
    ret.push("<li>" + changeLinkTargetLink(
    { newTarget: null,
...
function makeFixDab(data, navpop) {
  ...
  var rand_id = "waiting_" + String((new Date()).getTime()).replace(/\D/gi,'');
  var html = '<hr><div id="' + rand_id +'" class="waiting"><img src="' + rootUrl + 'waiting.gif" /></div>'; 
  listLinks(data, navpop, titleToEdit, "#" + rand_id);
  //var list=listLinks(data, navpop.originalArticle, titleToEdit);
  //if (list.length===0) { log('listLinks returned empty list'); return null; }
  //var html='<hr>' + popupString('Click to disambiguate this link to:') + '<ul><li>';
